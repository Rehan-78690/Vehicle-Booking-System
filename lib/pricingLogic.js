...
export async function calculatePrice(
    ...
    // Destination categories mapping
    ...
    // Get pricing rules for this vehicle type
    const pricingRules = await prisma.pricing.findMany({ where: { vehicle_type: vehicleType } });

    // Find the base rule
    const baseRule = pricingRules.find(
        r => r.destination_category === destCategory || r.destination_category === null
    );
    if (!baseRule) throw new Error(`No pricing rule found for ${vehicleType} in ${destCategory}`);

    // Calculate base price
    let basePrice = 0;
    switch (useCase) {
        case 'hourly-disposal':
            basePrice = hours ? (baseRule.rate_per_hour || 0) * hours : 0;
            break;
        case 'multi-day':
            basePrice = totalDays ? (baseRule.rate_per_day || 0) * totalDays : 0;
            break;
        case 'one-way':
            basePrice = distance ? (baseRule.rate_per_km || 0) * distance : 0;
            break;
        case 'day-excursion':
        case 'day-excursion-disposal':
            basePrice = (distance ? (baseRule.rate_per_km || 0) * distance : 0)
                + (hours ? (baseRule.rate_per_hour || 0) * hours : 0);
            break;
        default:
            throw new Error('Unsupported use case');
    }

    // Check if an adjustment rule applies
    let adjustmentValue = 0;
    let appliedConditionRule = null;
    const conditionRule = pricingRules.find(r => r.condition && r.vehicle_type === vehicleType);
    if (conditionRule && daysUntilTrip !== undefined) {
        if (daysUntilTrip < 3 && conditionRule.condition === 'Booking < 3 days') {
            adjustmentValue = conditionRule.adjustment || 0;
            appliedConditionRule = conditionRule;
        } else if (daysUntilTrip > 50 && conditionRule.condition === 'Booking > 50 days') {
            adjustmentValue = conditionRule.adjustment || 0;
            appliedConditionRule = conditionRule;
        }
    }

    const finalPrice = Math.round(basePrice * (1 + adjustmentValue) * 100) / 100;

    // Return both price and rule details
    return {
        totalPrice: finalPrice,
        appliedBaseRule: {
            vehicleType: baseRule.vehicle_type,
            destinationCategory: baseRule.destination_category,
            ratePerKm: baseRule.rate_per_km,
            ratePerHour: baseRule.rate_per_hour,
            ratePerDay: baseRule.rate_per_day
        },
        appliedConditionRule: appliedConditionRule
            ? {
                condition: appliedConditionRule.condition,
                adjustment: appliedConditionRule.adjustment
            }
            : null,
        calculationDetails: {
            useCase,
            ...
            daysUntilTrip,
            formulaUsed:
                useCase === 'one-way' ? `distance × rate_per_km`
                    : useCase === 'hourly-disposal' ? `hours × rate_per_hour`
                        : useCase === 'multi-day' ? `days × rate_per_day`
                            : `distance × rate_per_km + hours × rate_per_hour`
        }
    };
}
