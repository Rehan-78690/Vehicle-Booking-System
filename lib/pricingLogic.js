...
export async function calculatePrice(useCase, distance, bookingDays, hours, vehicleType = 'sedan') {
    const validUseCases = ['one-way', 'hourly-disposal', 'multi-day', 'day-excursion', 'day-excursion-disposal']; // Updated to match frontend
    ...
    // Validate vehicleType against existing rules
    const pricingRules = await prisma.pricing.findMany({ where: { vehicle_type: vehicleType } });
    if (pricingRules.length === 0) {
        throw new Error(`Invalid vehicle type: ${vehicleType}. No pricing rules found`);
    }

    try {
        const destinationCategory = 'Middle'; // In real app, determine based on cities
        const rule = pricingRules.find(
            (r) => r.vehicle_type === vehicleType && r.destination_category === destinationCategory
        );
        if (!rule) {
        ...
        switch (useCase) {
            ...
            case 'day-excursion-disposal':
                basePrice =
                    (distance ? rule.rate_per_km * (distance * 2) : 0) +
        ...
        if (conditionRule) {
            if (bookingDays < 1 && conditionRule.condition === 'Booking < 1 day')
                adjustment = conditionRule.adjustment || 0;
            else if (bookingDays >= 2 && bookingDays <= 3 && conditionRule.condition === 'Booking 2-3 days')
                adjustment = conditionRule.adjustment || 0;
            else if (bookingDays > 50 && conditionRule.condition === 'Booking > 50 days')
                adjustment = conditionRule.adjustment || 0;
        }
