...
export async function calculatePrice(useCase, distance, bookingDays, hours, vehicleType) {
    if (!vehicleType || (useCase !== 'one-way' && useCase !== 'disposal' && useCase !== 'multi-day' && useCase !== 'day-excursion' && useCase !== 'day-excursion-disposal')) {
        throw new Error('Invalid use case or vehicle type');
    }

    try {
        const pricingRules = await prisma.pricing.findMany({ where: { vehicle_type: vehicleType } });

        const rule = pricingRules.find(r => r.destination_category === 'Middle');
        if (!rule) throw new Error('No pricing rule found for vehicle type');
        ...
        switch (useCase) {
            ...
            case 'day-excursion-disposal':
                basePrice = (distance ? rule.rate_per_km * distance : 0) + (hours ? (rule.rate_per_hour || 0) * hours : 0);
                break;
            default:
